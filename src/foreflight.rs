...
impl ForeFlightCadenceProfile {
    pub fn validate(self) -> Result<Self> {
        ...
        if self.ahrs_interval != Duration::from_secs(1) / u32::from(self.ahrs_rate_hz) {
            return Err(Gdl90Error::InvalidField {
                field: "ForeFlight cadence profile",
                details: "AHRS interval must agree with its declared rate (nanosecond precision)"
                    .into(),
            });
        }
        Ok(self)
...
pub fn encode_datagram_with_limit(messages: &[Message], limit: usize) -> Result<Vec<u8>> {
    ...
    if limit == 0 || limit > crate::transport::MAX_UDP_PAYLOAD_SIZE {
        return Err(Gdl90Error::InvalidField {
            field: "ForeFlight datagram size limit",
            details:
                "must be in 1..=65507; path MTU is the caller's responsibility for custom limits"
                    .to_string(),
        });
    }
    ...
    for message in messages {
        ...
        if frame.len() > limit.saturating_sub(datagram.len()) {
            return Err(Gdl90Error::InvalidField {
                field: "ForeFlight datagram size",
                details: format!("encoded UDP payload exceeds the configured {limit}-byte limit"),
            });
        }
