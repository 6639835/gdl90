...
impl BandwidthManager {
    ...
    pub fn schedule(&self, mut inputs: ScheduleInputs) -> Result<ScheduleResult> {
        for candidate in inputs.alert_traffic.iter().chain(&inputs.proximate_traffic) {
            validate_range(candidate.range_nm)?;
        }
        for candidate in &inputs.uplinks {
            validate_range(candidate.station_range_nm)?;
            if candidate.time_slot > 31 {
                return Err(Gdl90Error::InvalidField {
                    field: "uplink time slot",
                    details: "must be in 0..=31".into(),
                });
            }
        }
...
fn validate_range(range_nm: f64) -> Result<()> {
    if !range_nm.is_finite() || range_nm < 0.0 {
        return Err(Gdl90Error::InvalidField {
            field: "candidate range",
            details: "must be finite and nonnegative".into(),
        });
    }
    Ok(())
}
