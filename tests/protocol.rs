...
fn nexrad_rle_block_round_trip() {
    ...
    assert_eq!(parsed.block.decode_bins().unwrap(), bins);
...
fn nexrad_block_reference_exposes_public_example_bits() {
    ...
    assert_eq!(reference.to_raw().unwrap(), [0x84, 0xA5, 0x70]);
...
fn nexrad_empty_bitmap_expands_referenced_blocks() {
    ...
    assert_eq!(block.to_payload().unwrap(), vec![0x04, 0xA5, 0x70, 0x00]);
}
