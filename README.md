...
- UAT uplink parsing and encoding for the documented structures
  ...
  - Generic Text APDUs and DLAC encode/decode with published-example coverage
...
src/
  ...
  storage.rs      Private atomic publication and file-permission helpers
...
See [Migration](docs/MIGRATION.md) for API and behavior changes, [Audit](docs/AUDIT.md) for findings and measured performance, and [Production deployment contract](docs/PRODUCTION_READINESS.md) before deployment.

Run `cargo bench --bench protocol` for reproducible local hot-path measurements. CI checks both debug and optimized builds because overflow and validation behavior must not depend on the build profile.
